...
export const CloudflareImagesRoute = () => {
  ...
  app.post('/api/cf-images/upload', async (c) => {
    try {
      ...
      const cfImages = new CloudflareImagesService(
        db,
        c.env.CF_ACCOUNT_ID,
        c.env.CF_IMAGES_API_TOKEN,
        c.env.CF_IMAGES_ACCOUNT_HASH
      )
  ...
  // 게시글 생성 (Cloudflare Images 통합)
  app.post('/api/cf-posts', async (c) => {
    try {
      const user = c.get('user')
      if (!user) {
        throw new AppError(ERROR_MESSAGES.AUTH.UNAUTHORIZED)
      ...
      const { blogId, content, ...postData } = data

      if (!blogId) {
        throw new AppError(ERROR_MESSAGES.GENERAL.BAD_REQUEST)
      }

      // Cloudflare Images로 외부 이미지 처리
      ...
      if (c.env.CF_ACCOUNT_ID && c.env.CF_IMAGES_API_TOKEN && c.env.CF_IMAGES_ACCOUNT_HASH) {
        ...
        processedContent = await cfImages.processContentImages(content, tempPostId)
