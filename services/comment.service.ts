...
import { blogs, comments, posts, user } from '../entities'
...
export const createComment = async (
    ...
    if (data.parentId) {
        const parentComment = await db
            .select()
            .from(comments)
            .where(and(eq(comments.id, data.parentId), eq(comments.postId, postId)))
            .get()
...
export const updateComment = async (
    ...
    const updatedComment = await db.update(comments).set(updateData).where(eq(comments.id, commentId)).returning().get()
...
export const deleteComment = async (db: DB, commentId: string, userId: string, isOwner?: boolean) => {
    ...
    const canDelete = commentWithPost.comment.userId === userId || (isOwner && commentWithPost.blog.userId === userId)

    if (!canDelete) {
        throw new AppError(ERROR_MESSAGES.COMMENT.UNAUTHORIZED)
    }

    const childComments = await db.select({ id: comments.id }).from(comments).where(eq(comments.parentId, commentId)).all()
...
export const getCommentsByPostId = async (
    ...
    const commentsData = await query.limit(limit).offset(offset).all()
...
export const getCommentCount = async (db: DB, postId: string) => {
    const result = await db.select({ count: count() }).from(comments).where(eq(comments.postId, postId)).get()
